...
class LruCache<K extends Object, V extends Object> {
  ...
  final DoubleLinkedQueue<(K, V)> _itemQueue = DoubleLinkedQueue<(K, V)>();

  @visibleForTesting
  DoubleLinkedQueue<(K, V)> get debugItemQueue => _itemQueue;
  ...
  final Map<K, DoubleLinkedQueueEntry<(K, V)>> _itemMap = <K, DoubleLinkedQueueEntry<(K, V)>>{};

  @visibleForTesting
  Map<K, DoubleLinkedQueueEntry<(K, V)>> get itemMap => _itemMap;
  ...
  void cache(K key, V value) {
    final DoubleLinkedQueueEntry<(K, V)>? item = _itemMap[key];
    if (item == null) {
      // New key-value pair, just add.
      _add(key, value);
    } else if (item.element.$2 != value) {
  ...
  void _removeLeastRecentlyUsedValue() {
    final bool didRemove = _itemMap.remove(_itemQueue.last.$1) != null;
    assert(didRemove);
